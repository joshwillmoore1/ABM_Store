...
class TestNDM: public AbstractCellBasedTestSuite
{
public:



 void Test2dBilayerRandomMotionForPolarisation()
    {
       ...
       {   
        ...
        simulator.SetOutputDirectory("Is Srn working?");
        ...
        //MAKE_PTR(MamDeltaNotchTrackingModifier<2>, p_modifier); // for fixed signal strenght
        simulator.AddSimulationModifier(p_modifier);

        MAKE_PTR(RandomMotionForce<2>, p_random_force);
        p_random_force->SetMovementParameter(0.0002); //0.1 causes dissasociation, 0.001 is not enough
    ...
    void Test2dBilayeProliferationForPolarisation()
    {
  ...
  //set up the geometry in a circle hex grid in cartesian coordinates
        ...
        for (unsigned i=0; i<mesh.GetNumNodes(); i++)
        {
            ...
            CellPtr p_cell(new Cell(p_state, p_cc_model));  
            p_cell->SetCellProliferativeType(p_Lneg_type);
            double birth_time = 1e5;
            p_cell->SetBirthTime(birth_time);
            cells.push_back(p_cell);
        }
       ...
       {   
             ...
             if (r <= lum_radius+5e-1)
                {
                        ...
                        if (RandomNumberGenerator::Instance()->ranf()>0.87)
                     {      
                        double birth = RandomNumberGenerator::Instance()->ranf()*(-4);
                        cell_iter->SetBirthTime(birth);
                        cout << "here"<< endl;
        ...
        //create the simulation 
        OffLatticeSimulation<2> simulator(cell_population);
        simulator.SetOutputDirectory("Test NDM Prolif 2D");
        simulator.SetSamplingTimestepMultiple(50);
        simulator.SetDt(0.002);
        ...
        MAKE_PTR(RandomMotionForce<2>, p_random_force);
        p_random_force->SetMovementParameter(0.0005); //0.1 causes dissasociation, 0.001 is not enough
        simulator.AddForce(p_random_force);
